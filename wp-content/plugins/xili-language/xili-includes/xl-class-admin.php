...
class xili_language_admin extends xili_language {
	...
	var $embedded_themes = array('twentyten', 'twentyeleven', 'twentytwelve', 'twentythirteen', 'twentyfourteen'); // bundled themes
	...
	function set_propagation_actions () {
		...
		if ( current_theme_supports( 'xiliml-authoring-rules' ) ) {
			$support = get_theme_support( 'xiliml-authoring-rules' );
	...
	function localize_admin_js( $case_news, $news_id ) {
		...
		switch ( $case_news ) {
			...
			case 'xl_new_version' :
				...
				$pointer_text .= '<p>' . esc_js( sprintf( __( 'This version %s improves compatibility with WP 4.0 when starting a very new website and is ready for 4.1-beta1 new install with 2015 theme. See also %s.','xili-language' ) , XILILANGUAGE_VER, '<a href="http://wordpress.org/plugins/xili-language/changelog/" title="'.$changelog.'" >'.$changelog.'</a>') ). '</p>';

				$pointer_text .= '<p>' . esc_js( sprintf( __( 'Previous version of %s improves new way/option to use flag of media asset and default from theme subfolder. See also %s.','xili-language' ) , XILILANGUAGE_VER, '<a href="http://wordpress.org/plugins/xili-language/changelog/" title="'.$changelog.'" >'.$changelog.'</a>') ). '</p>';
			...
			case 'add';
				...
				if ("" != $term ) {
					...
					if ( $doit ) {
						...
						$actiontype = "add";
						//$this->update_current_theme_support();
						$msg = 5;
...
// categories
	...
	function author_rules() {
		...
		//
		add_settings_section( 'option_section_settings_4', __('Other settings', 'xili-language'), array( $this, 'display_one_section'), $this->settings_authoring_settings .'_group');
		if ( current_theme_supports( 'widgets' ) ) {
			$field_args = array(
				...
				'id'			=> 'widget',
				'name'			=> 'widget',
				'desc'			=> __('If checked, xili-language widgets are available.', 'xili-language') ,
				'std'			=> 'enable',
				'label_for'		=> 'widget',
				'class'			=> 'css_class settings'
			);
...
// $this->xili_settings['functions_enable']
			$field_args = array(
				'option_name'	=> $this->settings_authoring_settings,
			...
			<div id="post-body" <?php echo $postbody_class; ?> >
				...
				<div id="post-body-content">
					...
					<h4><a href="http://dev.xiligroup.com/xili-language" title="xili-language page and docs" target="_blank" style="text-decoration:none" >
						...
						<a href="http://dev.xiligroup.com" target="_blank" title="<?php _e('Author'); ?>" >xiligroup.com</a>™ - msc 2007-2014 - v. <?php echo XILILANGUAGE_VER; ?>
					</h4>
	...
	function on_box_mail_content ( $data ) {
		...
		<label for="wplangenable">
			<input type="checkbox" id="wplangenable" name="wplangenable" value="enable" <?php if( $wplang == 'enable' ) echo 'checked="checked"' ?> />&nbsp;<?php echo "WPLANG= ".$this->get_WPLANG() ; ?>
		</label><br />
		<?php }
...
<?php
	...
	function add_language_attachment_fields ($form_fields, $post) {
		...
		if ( ! empty( $context ) && $context == 'custom_xili_flag' ) {
	...
	function add_display_media_states ( $media_states ) {
		...
		//if ( current_theme_supports( 'custom_xili_flag') ) {
			...
			$context = get_post_meta( $post->ID, '_wp_attachment_context', true );
			if ( ! empty( $context ) && $context == 'custom_xili_flag' && ! empty( $meta_header ) && $meta_header == $stylesheet )
				$media_states[] = __( 'Flag', 'xili-language' );
		//}
	...
	function flag_options_theme_page () {
		...
		<p><small><?php echo get_option("current_theme"); ?> by <a href="http://dev.xiligroup.com" target="_blank" >dev.xiligroup.com</a> (©2013) <?php echo '(xili-language v.' . XILILANGUAGE_VER . ')';?></small></p>
	...
	function display_flags_list ( $args ) {
		...
		$available = $this->get_flag_series();
		$listlanguages = $this->get_listlanguages () ;
		echo '<ul>';
	...
	function flag_theme_options_help_page () {
		...
		$help .= '<p>'.__('More detailled infos in future version.', 'xili-language').'</p>';
	...
	function restrict_manage_languages_posts () {
		...
		if ( in_array ( $post_type, $CPTs) ) {
			...
			<select name="<?php echo QUETAG ?>" id="<?php echo QUETAG ?>" class='postform'>
				...
				<option value="<?php echo LANG_UNDEF ?>" <?php echo ( isset ( $_GET[QUETAG] ) && $_GET[QUETAG] == LANG_UNDEF ) ? "selected=selected" : "" ; ?> > <?php echo $without; ?> </option>
