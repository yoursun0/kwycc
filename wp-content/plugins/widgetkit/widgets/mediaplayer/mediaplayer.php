	...
	*/
	public function site() {
		...
		if ($this->options->get('mediaplayer_enable', 1)) {
			...
			$call = "jQuery(function($){
				...
				$('%selector%').each(function(){
					...
					if (!ele.parent().hasClass('mejs-mediaelement')) {
						ele.data('mediaelement',new mejs.MediaElementPlayer(this, %options%));
			...
			$search = array("%selector%", "%options%");
			$replace = array($this->options->get('mediaplayer_selector', 'video,audio'), count($options) ? json_encode($options) : '{}');
			$call = str_replace($search, $replace, $call);
