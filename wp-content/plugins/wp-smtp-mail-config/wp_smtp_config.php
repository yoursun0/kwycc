<?php
/*
Plugin Name: WP-SMTP-MAIL-CONFIG
Version: 0.1.0
Plugin URI: http://tcss.co.in/wp-smtp-mail-config/
Description: configures the wp_mail() function to use SMTP instead of mail() and creates an options page to manage the settings.
Author: TakeCare Solutions & Services
Author URI: http://www.tcss.co.in
*/
 ...
 */
if (!function_exists('wp_smtp_mail_config_activate')) :
function wp_smtp_mail_config_activate() {
...
if (!function_exists('wp_smtp_mail_config_whitelist_options')) :
function wp_smtp_mail_config_whitelist_options($whitelist_options) {
...
function phpmailer_init_smtp($phpmailer) {
	...
	if (defined('WPMS_ON') && WPMS_ON) {
		...
		$phpmailer = apply_filters('wp_smtp_mail_config_custom_options', $phpmailer);
 ...
 */
if (!function_exists('wp_smtp_mail_config_options_page')) :
// Define the function
function wp_smtp_mail_config_options_page() {
	...
	if (isset($_POST['wpms_action']) && $_POST['wpms_action'] == __('Send Test', 'wp_smtp_mail_config') && isset($_POST['to'])) {
		
		// Set up the mail variables
		$to = $_POST['to'];
		$subject = 'WP Mail SMTP: ' . __('Test mail to ', 'wp_smtp_mail_config') . $to;
		$message = __('This is a test email generated by the WP Mail SMTP WordPress plugin.', 'wp_smtp_mail_config');
		...
		// Output the response
		?>
<div id="message" class="updated fade"><p><strong><?php _e('Test Message Sent', 'wp_smtp_mail_config'); ?></strong></p>
<p><?php _e('The result was:', 'wp_smtp_mail_config'); ?></p>
<pre><?php var_dump($result); ?></pre>
<p><?php _e('The full debugging output is shown below:', 'wp_smtp_mail_config'); ?></p>
<pre><?php var_dump($phpmailer); ?></pre>
<p><?php _e('The SMTP debugging output is shown below:', 'wp_smtp_mail_config'); ?></p>
<pre><?php echo $smtp_debug ?></pre>
	...
	?>
<div class="wrap">
<h2><?php _e('Advanced Email Options', 'wp_smtp_mail_config'); ?></h2>
<form method="post" action="options.php">
<?php wp_nonce_field('email-options'); ?>

<table class="optiontable form-table">
<tr valign="top">
<th scope="row"><label for="mail_from"><?php _e('From Email', 'wp_smtp_mail_config'); ?></label></th>
<td><input name="mail_from" type="text" id="mail_from" value="<?php print(get_option('mail_from')); ?>" size="40" class="regular-text" />
<span class="description"><?php _e('You can specify the email address that emails should be sent from. If you leave this blank, the default email will be used.', 'wp_smtp_mail_config'); if(get_option('db_version') < 6124) { print('<br /><span style="color: red;">'); _e('<strong>Please Note:</strong> You appear to be using a version of WordPress prior to 2.3. Please ignore the From Name field and instead enter Name&lt;email@domain.com&gt; in this field.', 'wp_smtp_mail_config'); print('</span>'); } ?></span></td>
</tr>
<tr valign="top">
<th scope="row"><label for="mail_from_name"><?php _e('From Name', 'wp_smtp_mail_config'); ?></label></th>
...
<span class="description"><?php _e('You can specify the name that emails should be sent from. If you leave this blank, the emails will be sent from WordPress.', 'wp_smtp_mail_config'); ?></span></td>
</tr>
</table>
...
<table class="optiontable form-table">
<tr valign="top">
<th scope="row"><?php _e('Mailer', 'wp_smtp_mail_config'); ?> </th>
<td><fieldset><legend class="screen-reader-text"><span><?php _e('Mailer', 'wp_smtp_mail_config'); ?></span></legend>
<p><input id="mailer_smtp" type="radio" name="mailer" value="smtp" <?php checked('smtp', get_option('mailer')); ?> />
<label for="mailer_smtp"><?php _e('Send all WordPress emails via SMTP.', 'wp_smtp_mail_config'); ?></label></p>
<p><input id="mailer_mail" type="radio" name="mailer" value="mail" <?php checked('mail', get_option('mailer')); ?> />
<label for="mailer_mail"><?php _e('Use the PHP mail() function to send emails.', 'wp_smtp_mail_config'); ?></label></p>
</fieldset></td>
</tr>
</table>
...
<table class="optiontable form-table">
<tr valign="top">
<th scope="row"><?php _e('Return Path', 'wp_smtp_mail_config'); ?> </th>
<td><fieldset><legend class="screen-reader-text"><span><?php _e('Return Path', 'wp_smtp_mail_config'); ?></span></legend><label for="mail_set_return_path">
...
<h3><?php _e('SMTP Options', 'wp_smtp_mail_config'); ?></h3>
<p><?php _e('These options only apply if you have chosen to send mail by SMTP above.', 'wp_smtp_mail_config'); ?></p>

<table class="optiontable form-table">
<tr valign="top">
<th scope="row"><label for="smtp_host"><?php _e('SMTP Host', 'wp_smtp_mail_config'); ?></label></th>
<td><input name="smtp_host" type="text" id="smtp_host" value="<?php print(get_option('smtp_host')); ?>" size="40" class="regular-text" /></td>
</tr>
<tr valign="top">
<th scope="row"><label for="smtp_port"><?php _e('SMTP Port', 'wp_smtp_mail_config'); ?></label></th>
<td><input name="smtp_port" type="text" id="smtp_port" value="<?php print(get_option('smtp_port')); ?>" size="6" class="regular-text" /></td>
</tr>
<tr valign="top">
<th scope="row"><?php _e('Encryption', 'wp_smtp_mail_config'); ?> </th>
<td><fieldset><legend class="screen-reader-text"><span><?php _e('Encryption', 'wp_smtp_mail_config'); ?></span></legend>
<input id="smtp_ssl_none" type="radio" name="smtp_ssl" value="none" <?php checked('none', get_option('smtp_ssl')); ?> />
<label for="smtp_ssl_none"><span><?php _e('No encryption.', 'wp_smtp_mail_config'); ?></span></label><br />
<input id="smtp_ssl_ssl" type="radio" name="smtp_ssl" value="ssl" <?php checked('ssl', get_option('smtp_ssl')); ?> />
<label for="smtp_ssl_ssl"><span><?php _e('Use SSL encryption.', 'wp_smtp_mail_config'); ?></span></label><br />
<input id="smtp_ssl_tls" type="radio" name="smtp_ssl" value="tls" <?php checked('tls', get_option('smtp_ssl')); ?> />
<label for="smtp_ssl_tls"><span><?php _e('Use TLS encryption. This is not the same as STARTTLS. For most servers SSL is the recommended option.', 'wp_smtp_mail_config'); ?></span></label>
</td>
</tr>
<tr valign="top">
<th scope="row"><?php _e('Authentication', 'wp_smtp_mail_config'); ?> </th>
<td>
<input id="smtp_auth_false" type="radio" name="smtp_auth" value="false" <?php checked('false', get_option('smtp_auth')); ?> />
<label for="smtp_auth_false"><span><?php _e('No: Do not use SMTP authentication.', 'wp_smtp_mail_config'); ?></span></label><br />
<input id="smtp_auth_true" type="radio" name="smtp_auth" value="true" <?php checked('true', get_option('smtp_auth')); ?> />
<label for="smtp_auth_true"><span><?php _e('Yes: Use SMTP authentication.', 'wp_smtp_mail_config'); ?></span></label><br />
<span class="description"><?php _e('If this is set to no, the values below are ignored.', 'wp_smtp_mail_config'); ?></span>
</td>
</tr>
<tr valign="top">
<th scope="row"><label for="smtp_user"><?php _e('Username', 'wp_smtp_mail_config'); ?></label></th>
<td><input name="smtp_user" type="text" id="smtp_user" value="<?php print(get_option('smtp_user')); ?>" size="40" class="code" /></td>
</tr>
<tr valign="top">
<th scope="row"><label for="smtp_pass"><?php _e('Password', 'wp_smtp_mail_config'); ?></label></th>
...
<h3><?php _e('Send a Test Email', 'wp_smtp_mail_config'); ?></h3>

<form method="POST" action="options-general.php?page=<?php echo plugin_basename(__FILE__); ?>">
<table class="optiontable form-table">
<tr valign="top">
<th scope="row"><label for="to"><?php _e('To:', 'wp_smtp_mail_config'); ?></label></th>
<td><input name="to" type="text" id="to" value="" size="40" class="code" />
<span class="description"><?php _e('Type an email address here and then click Send Test to generate a test email.', 'wp_smtp_mail_config'); ?></span></td>
</tr>
</table>
<p class="submit"><input type="submit" name="wpms_action" id="wpms_action" class="button-primary" value="<?php _e('Send Test', 'wp_smtp_mail_config'); ?>" /></p>
</form>

</div>
	<?php
	
} // End of wp_smtp_mail_config_options_page() function definition
endif;
 ...
 */
if (!function_exists('wp_smtp_mail_config_menus')) :
function wp_smtp_mail_config_menus() {
	
	if (function_exists('add_submenu_page')) {
		add_options_page(__('Advanced Email Options', 'wp_smtp_mail_config'),__('SMTP-Email', 'wp_smtp_mail_config'),'manage_options',__FILE__,'wp_smtp_mail_config_options_page');
	}
	
} // End of wp_smtp_mail_config_menus() function definition
endif;
 ...
 */
if (!function_exists('wp_smtp_mail_config_mail_from')) :
function wp_smtp_mail_config_mail_from ($orig) {
...
} // End of wp_smtp_mail_config_mail_from() function definition
endif;
 ...
 */
if (!function_exists('wp_smtp_mail_config_mail_from_name')) :
function wp_smtp_mail_config_mail_from_name ($orig) {
...
} // End of wp_smtp_mail_config_mail_from_name() function definition
endif;
...
function wp_mail_plugin_action_links( $links, $file ) {
	...
	$settings_link = '<a href="options-general.php?page=' . plugin_basename(__FILE__) . '">' . __( 'Settings', 'wp_smtp_mail_config' ) . '</a>';
...
if (!defined('WPMS_ON') || !WPMS_ON) {
	...
	add_filter('whitelist_options', 'wp_smtp_mail_config_whitelist_options');
	// Add the create pages options
	add_action('admin_menu','wp_smtp_mail_config_menus');
	// Add an activation hook for this plugin
	register_activation_hook(__FILE__,'wp_smtp_mail_config_activate');
...
add_filter('wp_mail_from','wp_smtp_mail_config_mail_from');
add_filter('wp_mail_from_name','wp_smtp_mail_config_mail_from_name');
